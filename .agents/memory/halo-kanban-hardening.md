...
- Server: `POST /client/:token/session` mints an httpOnly HMAC cookie (SESSION_SECRET); `clientAuth` middleware on `/client/:token` prefers cookie, but MUST pass through (next()) on GET when the token doesn't resolve — some /client routes serve paused accounts (billing resume). STRICT_MODE is ON (Jul 2026): mutating requests without a valid cookie get 401 "reopen your board link". The exchange + cookie↔path match use an ANY-STATUS resolver so paused accounts can still mint a cookie and resume billing; `req.propertyId` is only set for active accounts, handlers keep their own status gates. Every client-facing page (board/map/units/hub via useSessionExchange hook; halo ClientAdmin + ClientRequest inline) must POST /session on load — any NEW client-facing surface that mutates /client/:token must too, or its writes 401.
