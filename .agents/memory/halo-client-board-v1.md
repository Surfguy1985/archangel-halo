...
Ask this board is a **reasoning partner**: `askReason.ts` scores intent, resolves “that unit”, compares sites, and cites the clock. Cortex still ranks; the model only narrates. UI shows thinking steps, expandable why, citation pills, and proof tiles locked to the same focus (`askMedia.ts` — real Base44 before/after + Carto tiles, never invented). Server returns `{ answer, why, citations, followUps }`; invent-guard drops unknown units. Vacancy $ is the Pulse window (vacate → ready, property TZ). Do not embed HaloCommand. No HALO / Work App jargon on first run.
