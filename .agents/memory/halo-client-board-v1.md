...
Client Portfolio Pulse (`lib/board-ui` `PortfolioPulse`) matches the office Property Pulse HUD: full Carto map, **Overview top-left**, **Ask HALO** lime pill (64px) bottom-right. Header tells the three-desk story (`HaloLevelBar`: Portfolio = corporate, Pulse = property manager, Punchlist = Archangel Contractors). Public token boards lock the tiles (story only). Office `/portfolio` can jump to the three HUDs. Default-open is **Overview + Sites**. Primary nav is Overview / Sites / Needs — **no Crew, no Activity** (vendor stays on Punchlist). Overview uses Watch rings from already-computed `medianTurnDays` / `targetTurnDays` (not a second formula) plus a Base44 before/after pair (empty frames OK). Vacancy $ / turns / photos fold into Overview clicks. Keys: `halo_client_pulse_hud_open_v7` / `_mode_v7` / `_pos_v7`. Reset docks everything and restores Overview + Sites. Stage uses `isolation: isolate`; boxes `z-index` 1100+. Do not add `transform`/`filter`/`will-change` on the stage. Do not port GPS Finder / Site Twin / Twilio. Photos still come from Base44 `before`/`after` evidence. After `lib/board-ui` edits: `pnpm exec tsc --build lib/board-ui --force`.
