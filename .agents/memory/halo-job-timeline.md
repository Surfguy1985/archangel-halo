...
- Desktop JobFunnel (property detail) drives the 5-stage pipeline: crew → work → invoice → crew pay → close out.
...
- Manual crew assignment via PATCH /jobs/:id with a non-null crewLeaderId also (in one transaction) marks boardStatus "filled" and withdraws pending broadcasts — this sync lives server-side so voice/other clients get it free.
