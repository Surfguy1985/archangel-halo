...
- **Optional expiry:** pings can carry `expiresAt` (set via minutes preset at send). A per-minute scheduler sweep flips overdue open pings to status `cancelled` with `expiredAt` stamped (no new status enum value); targets go `pending → expired`. `expiredAt != null` is the "expired vs manual cancel" discriminator everywhere — portal shows expired pings (manual cancels stay hidden), Today surfaces "no one committed" while the job is uncrewed. The portal commit path also checks the deadline inside its transaction, closing the gap before the next sweep tick.
