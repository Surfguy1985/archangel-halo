 ...
 * (Falkon enterprise contract). HMAC delivery is not used.
 *
 ...
 *   X-Falkon-Client-Id:  <FALKON_CLIENT_ID>
 *   X-Falkon-Timestamp:  <epoch ms>
 *   X-Falkon-Nonce:      <uuid>
 *   X-Falkon-Signature:  <ed25519 base64url-no-padding>
 *
 * Missing Ed25519 key → markFailed (no HMAC fallback).
 *
...
import { getFalkonClientId } from "./falkonGateway";
 ...
 * Returns null if the private key is not available (caller must fail closed).
 */
  ...
  try {
    ...
    for (const event of pending) {
      ...
      if (ed25519Sig) {
        ...
        await markFailed(event.id, event.attempts, "Ed25519 signing key unavailable");
        continue;
      }
