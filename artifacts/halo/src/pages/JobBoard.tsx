      ...
      >
        ...
        <div className="p-[8px_20px_26px] overflow-y-auto">
          <SheetHeader className="text-left mb-[16px]">
            <SheetTitle className="font-display font-bold text-[19px] m-[6px_0_2px] text-[var(--ink)] drop-shadow-[0_0_8px_rgba(255,255,255,0.1)]">
              Broadcast Job
            </SheetTitle>
          ...
          <div className="space-y-4">
              ...
              {["all", "trade", "crews"].map((m) => (
                <button
                  ...
                  className={`flex-1 rounded-[10px] py-[8px] text-[13px] font-display font-bold capitalize transition-all ${
                    mode === m ? "bg-[var(--gold-light)] text-primary-foreground shadow-[0_0_10px_rgba(180,255,68,0.3)]" : "text-muted-foreground hover:bg-muted/50 hover:text-[var(--ink)]"
                  }`}
                >
                  {m}
                </button>
            ...
            {mode === "trade" && (
              <div className="bg-background border border-border rounded-[14px] p-[12px] shadow-sm">
                <div className="text-[13px] font-semibold mb-[8px] text-[var(--ink)]">Select Trade</div>
                <div className="flex flex-wrap gap-[6px]">
                  ...
                  {trades.map(t => (
                    <button
                      ...
                      className={`px-[12px] py-[6px] rounded-[8px] text-[13px] font-medium border transition-colors ${selectedTrade === t ? "bg-[var(--gold-light)] text-primary-foreground border-primary shadow-[0_0_8px_rgba(180,255,68,0.2)]" : "border-border text-muted-foreground hover:bg-muted/50 hover:text-[var(--ink)]"}`}
                    >
                      {t}
                    </button>
                  ))}
            ...
            {mode === "crews" && (
              <div className="bg-background border border-border rounded-[14px] p-[12px] shadow-sm max-h-[200px] overflow-y-auto">
                <div className="text-[13px] font-semibold mb-[8px] text-[var(--ink)]">Select Crews</div>
                <div className="flex flex-col gap-[6px]">
                  {crews?.map(c => (
                    <label key={c.id} className="flex items-center gap-[10px] p-[6px] hover:bg-muted/30 rounded-md cursor-pointer transition-colors group">
                      ...
                      />
                      <span className="text-[14px] font-medium text-[var(--ink)] group-hover:text-primary transition-colors">{c.name}</span>
            ...
            <div className="bg-background border border-border rounded-[14px] p-[12px] shadow-sm space-y-[12px]">
              <div>
                <div className="text-[13px] font-semibold mb-[8px] text-[var(--ink)]">Schedule Type</div>
                <div className="flex gap-[8px]">
                  ...
                  >
                    <div className="text-[13px] font-display font-bold text-[var(--ink)]">Set Schedule</div>
                  ...
                  >
                    <div className="text-[13px] font-display font-bold text-[var(--ink)]">Flex</div>
              ...
              <div className="flex gap-[10px]">
                {scheduleType === "flex" && (
                  <div className="flex-1">
                    <div className="text-[13px] font-semibold mb-[6px] text-[var(--ink)]">Finish within (days)</div>
                    <input
                      type="number"
                      min={1}
                      inputMode="numeric"
                      ...
                      className="w-full rounded-[10px] border border-border bg-card px-[12px] py-[9px] text-[14px] text-[var(--ink)]"
                    />
                  </div>
                )}
                <div className="flex-1">
                  <div className="text-[13px] font-semibold mb-[6px] text-[var(--ink)]">Crews needed</div>
                  <input
                    type="number"
                    min={1}
                    inputMode="numeric"
              ...
              {scheduleType === "flex" && (
                <div className="flex-1">
                  ...
                  <input
                    ...
                    value={flexDays}
                    onChange={(e) => setFlexDays(e.target.value)}
                    className="w-full rounded-[10px] bg-background border border-border px-[12px] py-[10px] text-[14px] text-[var(--ink)]"
                  />
                </div>
              )}
              <div className="flex-1">
  ...
  return (
    <div className="bg-card border border-border rounded-[20px] shadow-[0_0_20px_rgba(0,0,0,0.5)] overflow-hidden mb-[16px] transition-all hover:border-primary/30 group">
      <div className="p-[16px] border-b border-border">
        <div className="flex items-start justify-between mb-[8px]">
          <div>
            <Link href={`/jobs/${job.id}`}>
              <div className="font-display font-bold text-[18px] leading-[1.2] text-[var(--ink)] active:opacity-70 transition-opacity group-hover:text-primary">
            ...
            <div className="flex items-center gap-[8px] mt-[4px]">
              ...
              {job.category && (
                <span className="text-[11.5px] font-medium text-[var(--ink)] bg-muted px-[6px] py-[2px] rounded-[6px] border border-border">
                  {job.category}
                </span>
              )}
            </div>
        ...
        {job.description && (
          <div className="text-[13.5px] text-[var(--ink)]/80 mt-[10px] leading-snug line-clamp-3">
            {job.description}
          </div>
        )}
        ...
        <div className="flex items-center gap-[14px] mt-[12px] text-[12.5px] text-muted-foreground">
          {job.scheduledOn && (
            <div className="flex items-center gap-[4px]">
              ...
              <span className="text-[var(--ink)]">{new Date(job.scheduledOn).toLocaleDateString()}</span>
            </div>
          )}
          {job.crewLeaderName && (
            <div className="flex items-center gap-[4px]">
              <MapPin className="w-[14px] h-[14px] text-primary" />
              <span className="text-[var(--ink)]">{job.crewLeaderName}</span>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="space-y-[6px]">
            {priceItems.map(p => (
              <div key={p.id} className="flex items-start justify-between text-[13px]">
                ...
                <div className="font-display font-semibold tabular-nums shrink-0 text-[var(--ink)]">
              ...
              return (
                <div key={b.id} className="flex items-center justify-between text-[13px]">
                  <div className="flex items-center gap-[6px]">
                    ...
                    <span className="font-medium text-[var(--ink)]">{b.crewName}</span>
                  </div>
                  {b.respondedAt ? (
      ...
      <div className="p-[12px_16px] flex gap-[8px] bg-card">
        ...
        <button
          onClick={onEdit}
          aria-label="Edit job"
          className="rounded-[11px] px-[14px] py-[10px] text-[13px] font-display font-bold text-[var(--ink)] bg-background border border-border transition-colors active:scale-[0.98] flex items-center justify-center gap-[6px] hover:bg-muted hover:text-[var(--ink)]"
        >
  ...
  return (
    <div className="pt-2 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-[80px] px-2 flex flex-col min-h-screen">
      <div className="flex-1 w-full">
        <div className="font-display font-bold text-[26px] tracking-[-0.015em] leading-[1.1] mb-[2px] text-[var(--ink)] drop-shadow-[0_0_8px_rgba(255,255,255,0.1)]">Job Board</div>
          ...
          {[
            ...
            <button
              ...
              className={`flex-1 rounded-[10px] py-[8px] text-[13px] font-display font-bold transition-all ${
                filter === t.key ? "bg-[var(--gold-light)] text-primary-foreground shadow-[0_0_10px_rgba(180,255,68,0.3)]" : "text-muted-foreground hover:bg-muted/50 hover:text-[var(--ink)]"
              }`}
            >
              {t.label}
            </button>
        ...
        {isLoading ? (
           <div className="animate-pulse space-y-4">
             <div className="h-[200px] bg-card border border-border rounded-[20px]" />
             <div className="h-[200px] bg-card border border-border rounded-[20px]" />
           </div>
        ) : filteredData.length === 0 ? (
          <div className="text-center text-[13px] text-muted-foreground py-[60px] bg-card border border-border rounded-[20px] shadow-[0_0_15px_rgba(0,0,0,0.3)]">
