/**
 * Pulse Home — property view, map-first.
...
export default function PulseHome() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const qs = propertyId ? `?limit=40&propertyId=${encodeURIComponent(propertyId)}` : "?limit=40";
      const r = await fetch(`/api/pulse/home${qs}`, { credentials: "include" });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <header className="mx-auto max-w-2xl px-6 pb-2 pt-12">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 truncate text-[34px] font-semibold tracking-tight">
      ...
      <section className="mx-auto mt-8 grid max-w-2xl grid-cols-4 gap-2 px-6">
        {(["blocked", "turning", "waiting", "done"] as const).map((key) => (
          <CountChip
            key={key}
            label={key === "blocked" ? "Attention" : key === "done" ? "Ready" : key[0].toUpperCase() + key.slice(1)}
            value={
              key === "blocked"
                ? data?.counts.blocked ?? "—"
                : key === "turning"
        ...
        <section className="mx-auto mt-6 max-w-2xl px-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {data!.recentPhotoPaths.map((path, i) => (
              <div key={path + i} className="h-16 w-16 shrink-0 overflow-hidden rounded-[12px] bg-white/10">
                <img
                  src={path}
                  alt=""
...
function ViewToggle({ view, setView }: { view: "map" | "board"; setView: (v: "map" | "board") => void }) {
  return (
