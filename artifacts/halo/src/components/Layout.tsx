  ...
  return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center py-0 sm:py-7">
      <div className="hidden sm:block text-[12px] font-display font-semibold tracking-[0.22em] text-muted-foreground uppercase mb-[14px]">
        ARCHANGEL · <b className="text-[var(--gold-light)]">HALO</b> · OPERATIONS LAYER
      </div>
      ...
      <div className="w-full sm:w-[430px] h-[100dvh] sm:h-[850px] bg-background sm:rounded-[32px] sm:shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_30px_80px_rgba(0,0,0,0.5)] overflow-hidden relative flex flex-col">
        ...
        {/* Tab Bar */}
        <nav className="absolute left-0 right-0 bottom-0 h-[86px] bg-[rgba(7,16,30,0.92)] backdrop-blur-[20px] border-t border-[rgba(255,255,255,0.1)] flex items-start justify-around px-[6px] pt-[10px] pb-[calc(12px+env(safe-area-inset-bottom))]">
          <Link href="/">
            <button className={`flex flex-col items-center gap-[4px] w-[64px] text-[10px] font-bold tracking-[0.05em] pt-[5px] uppercase ${location === "/" ? "text-[var(--gold-light)]" : "text-[#8CA0B9] hover:text-white"}`}>
          ...
          <Link href="/properties">
            <button className={`flex flex-col items-center gap-[4px] w-[64px] text-[10px] font-bold tracking-[0.05em] pt-[5px] uppercase ${location.startsWith("/properties") ? "text-[var(--gold-light)]" : "text-[#8CA0B9] hover:text-white"}`}>
          ...
          <div className="w-[74px] relative -top-[26px]">
            <button 
              className="w-[66px] h-[66px] rounded-full relative grid place-items-center shadow-[0_0_20px_rgba(180,255,68,0.25)] mx-auto bg-[var(--gold-light)] text-[#07101E] hover:scale-105 active:scale-95 transition-transform"
          ...
          <Link href="/money">
            <button className={`flex flex-col items-center gap-[4px] w-[64px] text-[10px] font-bold tracking-[0.05em] pt-[5px] uppercase ${location.startsWith("/money") ? "text-[var(--gold-light)]" : "text-[#8CA0B9] hover:text-white"}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-[20px] h-[20px]">
          ...
          <Link href="/crews">
            <button className={`flex flex-col items-center gap-[4px] w-[64px] text-[10px] font-bold tracking-[0.05em] pt-[5px] uppercase ${location.startsWith("/crews") ? "text-[var(--gold-light)]" : "text-[#8CA0B9] hover:text-white"}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[20px] h-[20px]">
