/**
 * LiveMapCard — embedded Leaflet + OSM map in the conversation thread.
 *
 * Shows crew markers with name/status tooltip, last-ping timestamp,
 * colour-coded by checked-in / available status.
 * Centred on the bounding box of active crew or property cluster.
 * "Expand →" ghost link navigates to /map (the only secondary action).
 *
...
// ─── XSS-safe HTML escaper (used in Leaflet HTML strings) ────────────────────
...
function LeafletMap({ markers, center, zoom }: {
  ...
  useEffect(() => {
    ...
    Promise.all([
        ...
        const icon = L.divIcon({
          html: `
            <div style="
              ...
              background:${isIn ? "rgba(34,197,94,0.18)" : "rgba(59,130,246,0.15)"};
        ...
        L.marker([crew.lat, crew.lng], { icon })
          ...
          .bindPopup(`
            <div style="font-family:system-ui;padding:2px 0;min-width:120px">
              ...
              <div style="display:flex;align-items:center;gap:4px;font-size:11px;color:${isIn ? "#16a34a" : "#2563eb"}">
                <div style="width:6px;height:6px;border-radius:50%;background:${color}"></div>
                ${isIn ? "Checked in" : "Available"}
              </div>
      ...
      // Fit to markers if we have multiple
      if (markers.length > 1) {
        const group = L.latLngBounds(markers.map(m => [m.lat, m.lng] as [number, number]));
...
export function LiveMapCard({ query: _q }: LiveMapCardProps) {
  ...
  if (cLoading || pLoading) {
    return (
      <div
        ...
        style={{ border: "1px solid rgba(34,197,94,0.18)", background: "rgba(8,13,22,0.92)" }}
      >
        <div className="h-[260px] flex items-center justify-center gap-2.5">
          <Loader2 className="w-4 h-4 animate-spin" style={{ color: "rgba(34,197,94,0.5)" }} />
  ...
  // Build markers from crew GPS data
  const markers: CrewMarker[] = crewList
    .filter(c => {
      const lat = c.lastLat ?? c.lat ?? c.gpsLat;
      const lng = c.lastLng ?? c.lng ?? c.gpsLng;
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-white/[0.04] flex items-center gap-3">
        ...
        <button
          type="button"
          onClick={() => { refetch(); setMapKey(k => k + 1); }}
          className="w-7 h-7 grid place-items-center text-white/25 hover:text-white/55 transition-colors"
          title="Refresh"
        >
      ...
      <div style={{ height: 260, position: "relative" }}>
        {hasMap ? (
          <LeafletMap key={mapKey} markers={markers} center={center} zoom={zoom} />
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <div className="text-center">
              ...
              <div className="text-[11px] text-white/22 mt-1">
                Crew GPS pings appear here when they check in
              </div>
            </div>
            {/* Show crew list as fallback */}
            {crewList.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2 px-6 justify-center">
                {crewList.slice(0, 4).map(c => (
                  <div
                    key={c.id}
                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10.5px]"
                    style={{
                      background: (c.isCheckedIn || c.lastCheckinAt)
                        ? "rgba(34,197,94,0.10)"
                  ...
                  >
                    <div
                      ...
                      style={{ backgroundColor: (c.isCheckedIn || c.lastCheckinAt) ? "#22C55E" : "rgba(255,255,255,0.2)" }}
                    />
                    {c.name}
                  </div>
                ))}
        ...
        {/* Expand ghost link — bottom right */}
        <button
          type="button"
          ...
          style={{ background: "rgba(8,13,22,0.82)", backdropFilter: "blur(6px)" }}
        >
          Expand
