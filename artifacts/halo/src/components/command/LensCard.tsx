/**
 * LensCard — generative operations canvas.
 *
...
const LENS_META: Record<LensType, { label: string; icon: typeof DollarSign; color: string; deepLink: string; deepLinkLabel: string }> = {
  ...
  network:   { label: "Network",   icon: Users,      color: "#3B82F6", deepLink: "/crews",      deepLinkLabel: "Open Crew & Vendors" },
...
function KpiCell({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="bg-white/[0.048] rounded-[11px] px-3 py-2.5 flex flex-col gap-1 border border-white/5">
      <div className="text-[9.5px] font-bold tracking-[0.14em] uppercase text-white/30">{label}</div>
...
function JobRow({
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-3 bg-white/[0.042] hover:bg-white/[0.07] rounded-[11px] px-3 py-2.5 cursor-pointer transition-colors active:scale-[0.98]"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {job.jobNo && (
          <div className="text-[11px] text-white/38">{job.jobNo} · {job.scheduledOn ?? "Unscheduled"}</div>
        )}
      </div>
...
function LensEmpty({ icon: Icon, message }: { icon: typeof Camera; message: string }) {
  return (
    <div className="flex items-center gap-3 bg-white/[0.03] rounded-[12px] px-3 py-4">
      <Icon className="w-[15px] h-[15px] text-white/22 shrink-0" />
      <span className="text-[12.5px] text-white/42 leading-snug">{message}</span>
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      {unpaid.length > 0 ? (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Outstanding invoices</div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-white/38">{isOD ? "Overdue" : "Sent"} · #{inv.invoiceNo}</div>
                </div>
          ...
          {unpaid.length > 4 && (
            <button
              ...
              className="w-full text-center text-[11px] text-white/35 hover:text-white/55 py-1.5 transition-colors"
            >
        ...
        <div className="flex items-center gap-2.5 bg-[#22C55E]/8 rounded-[12px] px-3 py-3">
          ...
          <span className="text-[12.5px] text-[#22C55E]/85">All invoices are current.</span>
        </div>
      )}

      {paymentReady.length > 0 && (
        <div className="flex items-center gap-2.5 bg-[#B4FF44]/7 border border-[#B4FF44]/18 rounded-[12px] px-3 py-3">
          <CheckCircle2 className="w-[14px] h-[14px] text-[#B4FF44] shrink-0" />
          <span className="text-[12.5px] text-[#B4FF44]/85">
...
function TimelineLens({ query: _query }: { query?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {overdue.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-[#E11D48]/75 px-0.5">Overdue</div>
      ...
      {dueThisWeek.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Due this week</div>
...
function NetworkLens({ query: _query }: { query?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {crewList.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Crew</div>
          {crewList.slice(0, 4).map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-white/[0.042] rounded-[11px] px-3 py-2.5">
              <div className="w-7 h-7 rounded-full bg-[#3B82F6]/18 border border-[#3B82F6]/28 grid place-items-center shrink-0">
                <Users className="w-3 h-3 text-[#3B82F6]" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[12.5px] text-white/82 font-medium truncate">{c.name}</div>
                <div className="text-[11px] text-white/38">{c.trade ?? "General"}</div>
              </div>
              <div className="w-1.5 h-1.5 rounded-full bg-[#22C55E]" title="Available" />
            </div>
          ))}
          {crewList.length > 4 && (
            <div className="text-center text-[11px] text-white/28 py-1">+{crewList.length - 4} more crew</div>
          )}
        </div>
      )}

      {/* Falkon network teaser */}
      <button
        onClick={() => navigate("/falkon-network")}
        className="w-full flex items-center gap-2.5 bg-[#B4FF44]/7 border border-[#B4FF44]/18 rounded-[12px] px-3 py-3 text-left hover:bg-[#B4FF44]/10 transition-colors active:scale-[0.98]"
      >
        <div className="w-5 h-5 rounded-full bg-[#B4FF44]/18 grid place-items-center shrink-0">
          <TrendingUp className="w-2.5 h-2.5 text-[#B4FF44]" />
        </div>
        <span className="text-[12px] text-[#B4FF44]/75 flex-1 leading-snug">
          View Falkon Network — vendor COI, peers &amp; capacity
        </span>
        <ChevronRight className="w-3.5 h-3.5 text-[#B4FF44]/45 shrink-0" />
      </button>
    </div>
  );
}
...
function PortfolioLens({ query: _query }: { query?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {propList.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Properties</div>
          {propList.slice(0, 5).map(p => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-3 bg-white/[0.042] hover:bg-white/[0.07] rounded-[11px] px-3 py-2.5 cursor-pointer transition-colors active:scale-[0.98]"
              >
                <div className="w-7 h-7 rounded-[8px] bg-[#8B5CF6]/18 border border-[#8B5CF6]/28 grid place-items-center shrink-0">
                  <Building2 className="w-3 h-3 text-[#8B5CF6]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[12.5px] text-white/82 font-medium truncate">{p.name}</div>
                  <div className="text-[11px] text-white/38">{p.units ?? 0} units · {p.city ?? "—"}</div>
                </div>
                {propJobs.length > 0 && (
                  <span className="text-[10px] bg-[#B4FF44]/14 text-[#B4FF44] rounded-full px-2 py-0.5 font-bold shrink-0">
          ...
          {propList.length > 5 && (
            <div className="text-center text-[11px] text-white/28 py-1">+{propList.length - 5} more properties</div>
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <span className="text-[12.5px] text-[#22C55E]/80 flex-1">Live GPS map available in Crew Command Center</span>
        <ChevronRight className="w-3.5 h-3.5 text-[#22C55E]/45 shrink-0" />
      </div>

      {propList.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Property locations</div>
          {propList.slice(0, 5).map(p => (
            <div key={p.id} className="flex items-center gap-3 bg-white/[0.042] rounded-[11px] px-3 py-2.5">
              <div className="w-6 h-6 rounded-full bg-[#22C55E]/18 border border-[#22C55E]/28 grid place-items-center shrink-0">
                <MapPin className="w-2.5 h-2.5 text-[#22C55E]" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[12.5px] text-white/82 font-medium truncate">{p.name}</div>
                <div className="text-[11px] text-white/38">{p.city ?? "Location not set"}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {crewList.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Crew status</div>
          {crewList.slice(0, 4).map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-white/[0.042] rounded-[11px] px-3 py-2.5">
              <div className="w-2 h-2 rounded-full bg-[#22C55E] shrink-0" />
              <div className="text-[12.5px] text-white/82">{c.name}</div>
...
function EvidenceLens({ query: _query }: { query?: string }) {
  ...
  return (
    <div className="space-y-3">
      {recent.length > 0 ? (
        <>
          <div className="text-[9.5px] font-bold tracking-[0.16em] uppercase text-white/30 px-0.5">Recent photo evidence</div>
          <div className="grid grid-cols-3 gap-2">
            {recent.map(j => (
              <div
                key={j.id}
                onClick={() => navigate(`/jobs/${j.id}`)}
                className="aspect-square bg-white/[0.06] rounded-[11px] border border-white/7 grid place-items-center cursor-pointer hover:bg-white/[0.1] transition-colors relative overflow-hidden"
              >
                <Camera className="w-5 h-5 text-white/25" />
                <div className="absolute bottom-1.5 right-1.5 text-[9px] bg-black/65 text-white/65 rounded-full px-1.5 py-0.5 tabular-nums">
                  {(j as { photoCount?: number }).photoCount ?? "—"}
                </div>
              </div>
            ))}
          </div>
          <div className="text-center text-[11px] text-white/28">
            {jobsWithPhotos.length} jobs with evidence — tap to view
          </div>
        </>
      ) : (
...
export function LensCard({ lensType, query, onDeepLink }: LensCardProps) {
  ...
  return (
    <div
      className="w-full rounded-[18px] overflow-hidden mb-3 shadow-[0_6px_24px_rgba(0,0,0,0.28)]"
      style={{ border: `1px solid ${meta.color}1E` }}
    >
      {/* Lens header */}
      <button
        onClick={() => setExpanded(e => !e)}
        className="w-full flex items-center gap-3 px-4 py-3 bg-[#080F1D] hover:bg-[#0B1525] transition-colors"
      >
        <div
        ...
        <div className="flex-1 text-left">
          ...
          {query && (
            <div className="text-[10.5px] text-white/30 truncate mt-0.5">"{query}"</div>
          )}
        </div>
        {expanded
      ...
      {/* Lens content */}
      {expanded && (
        <div className="bg-[#060D1B] px-4 py-3.5">
          ...
          <button
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 text-[11px] font-bold tracking-[0.1em] uppercase py-2.5 rounded-[11px] border border-white/7 text-white/28 hover:text-white/55 hover:border-white/12 hover:bg-white/[0.03] transition-all active:scale-[0.98]"
          >
            {meta.deepLinkLabel}
