# Migrating Expo UI to HALO Design System

Read `artifacts/halo-ds/docs/AGENTS.md` and
`artifacts/halo-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/halo-ds/lib/native-theme` and
  `@workspace/halo-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/halo-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/halo-ds` and retains only
