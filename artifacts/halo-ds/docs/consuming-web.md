# Consuming HALO Design System in web apps

Read `artifacts/halo-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/halo-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/halo-ds/styles.css";
```
...
`node_modules/@workspace/halo-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/halo-ds/components/ui/button";
import { cn } from "@workspace/halo-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/halo-ds/hooks/use-toast";
```
...
`@workspace/halo-ds/components/ui/button`. Run the app's typecheck
